...
everything else plugs into it), `strategy.py` (reusable routing and budget
...
| **`strategy.py`** | Reusable policy for successor routing, narrow masks, budget pacing, caching, replica choice, and rate-aware A2A delegation. `gateway.py` imports the pieces needed for authoritative enforcement. | The same `command` / `tool_call` events as `gateway.py`, plus the credit trajectory (`GatewayContext.credits` across all 10 rounds). | `wasteful` **3** · `protocol_misuse` **6** (lease/mask discipline) · `stale_read` **8** (replica choice) |
| **`guardrails.py`** | Validates exchange-local grounding, bilingual prompt injection, private-value redaction, numeric support, required fields, and conflict disclosure. `safe_answer_from_trace` builds the citation allow-list only from this exchange's `tool_result` events, removes unreturned citations, and abstains when the repaired draft is no longer grounded. | The `answer` L1 event (`text`, `cited_anchors`) the arena hands your opponent's prosecutor on prosecution (CONTRACTS.md section 5.4). | `ungrounded` **5** · `fabricated_citation` **8** · `hallucination` **7** · `guardrail_breach` **8** · `privacy_leak` **8** · `unsupported_precision` **4** · partial credit toward `non_responsive` **4** / `incoherent` **4** via abstaining instead of guessing |
...
| **`prompt.md`** | The model's live reasoning: explicit routes for all eight ask types, tool policy, citation contract, refusal policy, conflict handling, and mandatory preflight before `ANSWER`. Layered ON TOP of `kit.loop.prompt`'s harness grammar. | `model_turn.action_raw` — indirectly everything, since every `Command` and final `ANSWER` trace back to what the model chose. | `unflagged_conflict` **6** · `overreach` **5** · `incoherent` **4** · `non_responsive` **4** · semantic defence against `guardrail_breach` **8** |
