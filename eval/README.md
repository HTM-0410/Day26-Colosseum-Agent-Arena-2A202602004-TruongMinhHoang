...
The acceptance suite additionally runs the trace-native answer policy over 30 deterministic seeds
and 10 rounds per seed. Its citation allow-list is built only from `tool_result` events in the
current exchange; the check fails if any injected, unreturned citation survives finalisation.
